...
def saveDataBlockChain(currentData): #calling to save data in blockchain
    ...
    deployed_contract_address = '0x2A33Ee7d5Ee0a2c38dD0D9275f0537ec8DfddEd8'
...
def readDetails(): #calling to read data from blockchain
    ...
    deployed_contract_address = '0x2A33Ee7d5Ee0a2c38dD0D9275f0537ec8DfddEd8' #hash address to access industrail contract
    with open(compiled_contract_path) as file:
        contract_json = json.load(file)  # load contract info as JSON
        contract_abi = contract_json['abi']  # fetch contract's abi - necessary to call its functions
    file.close()
